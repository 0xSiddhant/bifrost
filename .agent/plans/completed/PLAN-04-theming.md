...
- **Schema is the contract:** JSON Schema (draft 2020-12) validated with **ajv** on every load and on every future upload. Required: `name`, `id`, `mode` (`dark|light`), full color role set; optional: fonts (must reference self-hosted families only), radii, spacing overrides, **and a `syntax` group (`--syn-key/string/number/bool/null/punct`, consumed by Runestone/PLAN-07 and the JSON Differ/PLAN-08; when omitted, defaults are derived from `--accent`/`--text` so no theme breaks)**. Colors validated as hex/rgb(a); contrast lint (warn if `--text` vs `--bg` < 4.5:1) — warn, don't block, but Heimdall shows the warning.
...
- [ ] Add `--syn-*` syntax tokens to `tokens.css` with Aurora + Daybreak values, plus the derived-defaults logic for themes omitting the `syntax` group (consumed by PLAN-07/08)
