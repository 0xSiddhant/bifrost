...
12. `npm run logs` **actually tails a file that exists** (the pre-existing `app.log` path bug is fixed — **this half landed early in PR #39**) and renders `INFO`/`WARN`/`ERROR` with colour on new lines; dev-mode stdout does the same — the no-Docker fallback is intact, not silently degraded. The rendering half is **still open**: it depends on `levelKey: 'logLevel'` (Step 1 item 4), so re-verify after that lands.
