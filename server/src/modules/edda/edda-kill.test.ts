...
const storageRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'bifrost-edda-kill-'));
...
describe('edda save-burst kill test', () => {
  it('SIGINT during a save burst leaves no torn rows', { timeout: 60_000 }, async () => {
    ...
    const listResponse = await fetch(`${BASE}/api/edda?limit=500`);
    ...
    for (const doc of acknowledged) {
      ...
      const full = await fetch(`${BASE}/api/edda/${doc.slug}`);
