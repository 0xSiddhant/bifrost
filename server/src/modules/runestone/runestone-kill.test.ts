/**
 * PLAN-07 kill test: SIGINT the real server mid save-burst, restart it, and
 * prove no torn rows — every save the server acknowledged (201) is fully
 * present and valid JSON afterwards; unacknowledged saves are absent, not
 * half-written. WAL + synchronous better-sqlite3 writes make this hold.
 */
...
const storageRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'bifrost-rune-kill-'));
...
describe('runestone save-burst kill test', () => {
  it('SIGINT during a save burst leaves no torn rows', { timeout: 60_000 }, async () => {
    ...
    const listResponse = await fetch(`${BASE}/api/runestone?limit=500`);
    ...
    for (const doc of acknowledged) {
      ...
      const full = await fetch(`${BASE}/api/runestone/${doc.slug}`);
