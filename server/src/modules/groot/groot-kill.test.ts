/**
 * PLAN-19 kill test: SIGINT the real server mid save-burst, restart it, and
 ...
 *
 * The documents carry comments and anchors on purpose: "fully present" for a
 * YAML document means the exact bytes, since the whole promise of this feature
 * is that a stored document is handed back as written.
 */
...
const contentFor = (index: number): string =>
  ['# burst document', `name: burst-${index}`, 'base: &b', '  replicas: 2', 'prod:', '  <<: *b', ''].join(
    '\n',
  );
