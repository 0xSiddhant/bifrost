/**
 * Relic-name generator (PLAN-07): "<flavor-adjective> <Name>" titles like
 * "Gleaming Gungnir" — used for runestone document titles now, default file
 * names in Part B, and available to any future module.
 *
 * Mirrored on the client as client/src/core/relicNames.ts — keep in sync.
 *
 * Not to be confused with client/src/assets/relics/ (background line art), and
 * deliberately separate from presence's character-name pool (merged code with
 * per-device uniqueness semantics; this bank is per-title).
 */
