...
- A version-mismatch check against the GitHub Releases API (never npm — there is no registry), throttled to once per 6 hours and run only from `bifrost update` and `bifrost doctor` — never as a passive notice ahead of other commands — plus an explicit `bifrost update` command that performs the update when the user actually runs it — see Decisions.
...
- Any version check running ahead of dispatch for commands other than `update`/`doctor`, and the check ever mutating anything itself. `bifrost doctor` only ever reports a mismatch; `bifrost update` is the one explicit, user-run command that installs — see the Update check decision below.
...
### Update check + `bifrost update`: against GitHub Releases, not npm — checked only from `update`/`doctor`, throttled to 6 hours, on mismatch not ordering

Distribution is GitHub-Releases-tarball-only (above), so "is there a version to sync to" has to mean "check GitHub's Releases API," not npm's registry — `update-notifier` (the library the earlier draft of this plan leaned on) is npm-registry-specific and doesn't apply here, and there's no equally standard off-the-shelf library for "check a GitHub repo's releases against my own version." This is hand-rolled, the same call already made for `config.ts`'s XDG path: a single unauthenticated `GET https://api.github.com/repos/0xSiddhant/bifrost/releases/latest` (confirmed real repo slug, `git remote -v`; public repo, no token needed) returns `tag_name` (`v1.4.0`) and an `assets` array to find the `bifrost-cli-*.tgz` entry's `browser_download_url` in — a JSON GET and a plain version-string equality check, not fiddly enough to justify a dependency.

**Checked only from `bifrost update` and `bifrost doctor` — never as a passive notice ahead of other commands.** An earlier draft of this plan had `index.ts` fire a background check ahead of every command's dispatch; that's gone. `push`/`pull`/`clip`/etc. never touch the GitHub API at all, so there's no unexpected network dependency on commands that have nothing to do with updating, and no notice text to ever suppress under `--json`/non-TTY. The two commands that do care share one check: `update` needs to know whether to install; `doctor` reports it as one more diagnostic line.

**New `core/selfUpdate.ts`**: `checkLatest()` fetches `GET .../releases/latest` and caches the result — `{ lastCheckedAt, latestVersion }`, held in the same config file `config.ts` already owns — and is **throttled to 6 hours**: a call within 6 hours of `lastCheckedAt` returns the cached `latestVersion` with no network request; only once 6 hours have elapsed does the next call actually hit GitHub and refresh the cache. This is deliberately coarser than "always fresh" — GitHub's unauthenticated rate limit is 60 requests/hour, and sharing one 6-hour cache between `update` and `doctor` means neither can be scripted into hammering that limit. The tradeoff is stated plainly, not glossed over: a release published minutes ago may not be visible to either command for up to 6 hours after the last check.

**Mismatch, not ordering.** The comparison is a plain string check — installed version equals the cached `latestVersion`, or it doesn't — not a semver `<`/`>` compare. That matches what both callers actually need: `performUpdate()` only needs "is there anything to do," and `doctor` only needs "do these two strings agree." A mismatch is reported as `installed vX.Y.Z, latest published vA.B.C` — deliberately not phrased as "a newer version is available," since a mismatch alone doesn't say which direction it runs. The one realistic case where it runs backwards is the owner's own dev machine: `cli-sync.ts` (below) re-syncs the global `bifrost` from whatever's on `develop` on every `npm run build`/`start`, so a locally-installed version can be genuinely ahead of the last published tag while the next release is still being worked on. Neutral wording means that case reads as "these differ," not a wrong "update available" telling the owner to downgrade.

`performUpdate()` resolves the release asset's URL and shells out to `npm install -g <url>`, surfacing npm's own stdout/stderr directly on failure rather than swallowing it — an `npm install -g` can fail for reasons (permissions, network) that are npm's to explain, not this plan's to reinterpret. On a match, it no-ops cleanly ("already on the latest version") rather than reinstalling identical bits.
...
**`bifrost update` is the one explicit, user-run exception to "the CLI never mutates its own install silently."** `doctor` only ever reports a mismatch; running `bifrost update` is the opt-in action that actually installs — consistent with, not a reversal of, the Scope exclusion above.
...
- [ ] `selfUpdate.ts` (+ `selfUpdate.test.ts`): `checkLatest()` against `GET https://api.github.com/repos/0xSiddhant/bifrost/releases/latest`, throttled to a 6-hour cache in `config.ts`'s config file (a call inside the window returns the cached version with no network request); compares by plain string equality, not semver ordering; called only from `update`/`doctor`, never ahead of other commands' dispatch; `performUpdate()` resolves the `bifrost-cli-*.tgz` asset URL and shells out to `npm install -g <url>`, surfacing npm's own error output on failure
...
- [ ] `doctor.ts`: sequences config/discover/client/capabilities/GitHub-reachability-and-version-mismatch (via `selfUpdate.ts`'s throttled `checkLatest()`)/Node-version checks into a ✓/✗ report reusing each layer's own remediation wording; the GitHub check is informational only and never affects the exit code; `--json` emits the same checks structured
...
18. `push`/`pull`/`clip`/etc. never call the GitHub API — confirmed by request count, not just the absence of a notice. With a version mismatch against a real published GitHub Release, `bifrost doctor` reports it (installed vX vs. latest vY) without affecting its exit code. `bifrost update` then installs the real latest version, verified by `bifrost --version` afterward; running it again immediately reports already up to date and performs no reinstall. Running `update` or `doctor` a second time within 6 hours reuses the cached GitHub result rather than issuing a fresh request (verified by request count); past the 6-hour window, the next call refreshes it.
...
20. `bifrost doctor` against a real, reachable server, with the installed CLI matching the latest GitHub release, reports all six checks passing. Pointing `--host` at an unreachable address reports the host-resolution/server-reachability checks as failing, using the same remediation text `status`/`pull`/etc. would themselves show, and the command exits non-zero. A deliberately version-mismatched install reports the GitHub check as a mismatch warning naming both version strings, and disconnecting from the internet (GitHub unreachable) is reported the same way — informational either way, exit code unaffected.
...
- [ ] Unit `core/selfUpdate.test.ts` — mocked GitHub API responses: mismatch, exact match, a call inside the 6-hour throttle window skipping the network call entirely and returning the cached version, a call past the window refreshing it, and a release with no matching `bifrost-cli-*.tgz` asset handled as a clean error rather than a crash
...
- [ ] Manual: install an older real release's CLI tarball, confirm `bifrost doctor` reports the version mismatch, then run `bifrost update` and confirm it actually installs the current latest — followed by a second run confirming the clean "already up to date" no-op, and confirm `push`/`pull`/etc. never hit the GitHub API in the meantime
